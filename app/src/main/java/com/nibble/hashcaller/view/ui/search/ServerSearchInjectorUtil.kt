...
object ServerSearchInjectorUtil {
    fun provideViewModelFactory(firebaseUser: FirebaseUser?, context:Context): ServerSearchViewmodelFactory {
        ...
        return ServerSearchViewmodelFactory(SearchNetworkRepository(TokenHelper(firebaseUser), callersInfoFromServerDAO), LibPhoneCodeHelper(PhoneNumberUtil.getInstance()))
    }
}