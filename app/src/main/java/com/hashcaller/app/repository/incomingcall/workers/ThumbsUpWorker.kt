...
class ThumbsUpWorker(private val context: Context, private val params: WorkerParameters) :
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            val formatedNum = libCountryHelper.getES164Formatednumber(
                formatPhoneNumber(inputData.getString(
                    NUMBER
                )!!), countryCodeIso)
            val hashedNum  = Secrets().managecipher(null, formatedNum)
        val response = repository.upvote(
            SuggestNameModel(
